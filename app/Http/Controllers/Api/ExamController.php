...
class ExamController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        // 1. Cek Sesi
        $sesi = SesiUjian::where('is_active', true);
        
        // Jika token diwajibkan, filter berdasarkan token
        if ($request->filled('token')) {
            $sesi->where('token', strtoupper($request->token));
        }

        $sesi = $sesi->first();

        if (!$sesi) {
            return response()->json(['message' => 'Sesi tidak ditemukan atau token salah.'], 422);
        }

        // Jika sesi mewajibkan token tapi request tidak ada token
        if ($sesi->use_token && !$request->filled('token')) {
            return response()->json(['message' => 'Token ujian wajib diisi untuk sesi ini.'], 422);
        }
